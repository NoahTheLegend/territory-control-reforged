...
void onTick(CBlob@ this)
{
    ...
    if (isServer() && type == 0 && this.exists("detach_timing") && this.get_u32("detach_timing") + 1 >= getGameTime())
        this.AddTorque(this.getVelocity().x * this.getMass() * (1.0f+(XORRandom(50) * 0.01f)));

    s32 heat = this.get_s32("heat");
    s32 maxheat = this.get_s32("max_heat");
    f32 heatscale = float(heat) / float(maxheat);

    if (isClient() && heat > 0 && getGameTime() % (int((1.0f - heatscale) * 9.0f) + 1) == 0)
    {
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
    ...
    if (isServer())
    {
        ...
        if (XORRandom(2) == 0) MakeMat(hitterBlob, worldPoint, "mat_plasteel", (XORRandom(5)) * damage);
    }

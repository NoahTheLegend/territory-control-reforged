...
void onInit(CBlob@ this)
{
	...
	settings.B_GRAV = Vec2f(0, 0.0015f); //Bullet gravity drop
	settings.B_SPEED = 95; //Bullet speed, STRONGLY AFFECTED/EFFECTS B_GRAV
	settings.B_TTL = 14; //TTL = 'Time To Live' which determines the time the bullet lasts before despawning
	settings.B_DAMAGE = 0.65f; //1 is 1 heart
	settings.B_TYPE = HittersTC::bullet_low_cal; //Type of bullet the gun shoots | hitter

	//Recoil
	settings.G_RECOIL = -6; //0 is default, adds recoil aiming up
	...
	//Sound
	settings.FIRE_SOUND = "mp40.ogg"; //Sound when shooting
	//settings.RELOAD_SOUND = "SMGReload.ogg"; //Sound when reloading

	//Offset
	settings.MUZZLE_OFFSET = Vec2f(-15, -4.25f); //Where the muzzle flash appears

...
const string[] material_tooltips = { // tooltips for materials
	...
	"Requires a refrigerant and heat "+material_generation_ranges[1][0]+" - "+material_generation_ranges[1][1]+"°C",
	"Requires heat "+material_generation_ranges[2][0]+" - "+material_generation_ranges[2][1]+"°C",
	"Requires a catalyzer and heat "+material_generation_ranges[3][0]+" - "+material_generation_ranges[3][1]+"°C"
};
...
void onInit(CBlob@ this)
{
	...
	AddIconToken(material_icons[1], "ReactorRequirements.png", Vec2f(48, 16), 2);
	AddIconToken(material_icons[2], "ReactorRequirements.png", Vec2f(48, 16), 3);
	AddIconToken(material_icons[3], "ReactorRequirements.png", Vec2f(48, 16), 4);
...
void SetButtons(CBlob@ this, bool register = true)
{
	...
	if (register)
	{
		...
		u8 buttons_count = 11;
		Vec2f size = Vec2f(tsize.x / buttons_count, screen_center.y + terminal_size.y / 2 - pos.y - footer_height);
		...
		{Button@ button = Button(net_id, "Turn on/off", "", 0, Vec2f(16, 16), 1, Vec2f_zero, "Turn on", pos + Vec2f(terminal_size.x - size.x * 2, 0), Vec2f(size.x * 2, size.y), "switch", SColor(255, 100, 155, 255));
		registerButton(this, @button);}
	}
	else // update positions
	{
		...
		u8 buttons_count = 11;
		Vec2f size = Vec2f(tsize.x / buttons_count, br.y - pos.y - footer_height) * side_height_factor;
		...
		button.pos = Vec2f_lerp(pos, pos + Vec2f(tsize.x - size.x * 2 - 2, 0), side_height_factor);
		button.size = Vec2f(size.x * 2, size.y);
		...
		button.icon_offset = has_catalyzer ? Vec2f(10, 16) * factor : Vec2f(1, 0) * factor;
		...
		button.icon_offset = has_refrigerant ? Vec2f(10, 16) * factor : Vec2f(1, 0) * factor;
		...
		button.size = size;
		button.icon_offset = temp_mode == 0 ? Vec2f(9, 15) * factor : Vec2f(0, 0);
		...
		button.size = size;
		button.icon_offset = temp_mode == 1 ? Vec2f(9, 15) * factor : Vec2f(0, 0);
		...
		button.size = size;
		button.icon_offset = temp_mode == 2 ? Vec2f(9, 15) * factor : Vec2f(0, 0);

...
void DrawInventoryOnHUD(CBlob@ this, Vec2f tl, Vec2f hudPos = Vec2f(0, 0))
{
	...
	for (int i = 0; i < itemsToShow.length; i++)
	{
		...
		if (quantity >= item.maxQuantity * 1.0f) col = SColor(255, 255, 255, 255);
		else if (quantity >= item.maxQuantity * 0.5f) col = SColor(255, 255, 255, 128);
		else if (quantity >= item.maxQuantity * 0.25f) col = SColor(255, 255, 155, 55);
		...
		if (quantity != 1)
		{
			...
			GUI::SetFont("menu");
			GUI::DrawText(""+quantity, itempos + Vec2f(xOffset, -5), col);
		}
	}
}
